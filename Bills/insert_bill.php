...
for ($i = 0; $i < count($keep); $i++) {
    if ("B000" != $keep[$i]) {
        ...
        $sql = "INSERT INTO bills VALUES ('B000','$cus_id','$today','$date','$all_stock','$discount_price')";
...
if ($not_B_zero == false) { // 
    ...
    $fill_num = 0;
    $boo_keep_last = false;
    $sql = "SELECT bill_id FROM bills ORDER BY bill_id ASC";
    $result = mysqli_query($con, $sql);
    $pr_id = sprintf("%03d", mysqli_num_rows($result));
    $bill_id =  "B" . $pr_id;
    if ($bill_id == "B001") {  //ไม่รู้จะอธิบายยังไง เอาเป็นว่าถ้าไม่มี มีปัญหาแน่นอน 
        $sql = "INSERT INTO bills VALUES ('B001','$cus_id','$today','$date','$all_stock','$discount_price')";
        mysqli_query($con, $sql);
    } else {
        ...
        if ($boo_keep_last == false) {  //ใช้เพื่อเพิ่มตำแหน่งระหว่าง 1 to (N-1) ตัวที่หายไป
            ...
            $bill_id =  "B" . $pr_id;
            $sql = "INSERT INTO bills VALUES ('$bill_id','$cus_id','$today','$date','$all_stock','$discount_price')";
            mysqli_query($con, $sql);
        } else {
